# E-CROSS
This repository contains the optimisation code and the origin-destination (OD) electric vehicle data of the publication by Eggimann and Romano (2026).
...
We refer to the publication for a detailed methodological description of:
...
## System requirements
The optimisation was performed in Python (Version 3.10), using the Pyomo package and the 'CBC' solver. No non-standard hardware is necessary to replicate the simulations.
...
2. The information on the trips and electric vehicle traffic needs to be provided in an origin-destination `.csv` file. Here, the OD-matrix of the simulation is made available (`trip_OD_matrix.csv `) that includes information on the smulated numbers of electric vehicles. The distance calculations are not provided; they were performed using the [Google API](https://developers.google.com/maps/documentation/directions).  
